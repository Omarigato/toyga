...
import { getTemplates, Template } from '@/lib/apiClient';
...
export default function PhotoTemplatesSection() {
    ...
    useEffect(() => {
        getTemplates()
            .then((data) => {
                // Slice first 6 templates for display on homepage
            ...
            <div className="w-full px-4 md:px-6 lg:px-8">
                ...
                {loading ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-6xl mx-auto">
                        {[1, 2, 3, 4, 5, 6].map((i) => (
                            <div key={i} className="flex flex-col gap-3 rounded-xl overflow-hidden border border-background-200/70 p-4">
                                ...
                                <div className="flex flex-col gap-3 p-4 md:p-5">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        <span className="text-xs text-foreground-600">
                                            {template.extra_price ? `+ ${template.extra_price} ₸ жаңарту` : '+ 400 ₸ жаңарту'}
                                        </span>
                                    </div>
                                    <a
                                        href={`https://api.whatsapp.com/send?phone=${WHATSAPP_NUMBER}&text=${encodeURIComponent(`Сәлеметсіз бе! "${template.title}" шақыру үлгісіне тапсырыс бергім келеді.`)}`}
                                        rel="nofollow"
                                        target="_blank"
